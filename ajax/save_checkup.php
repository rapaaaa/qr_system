<?php
	...
	$fetch_checkup 	= $mysqli->query("SELECT * FROM check_ups WHERE app_id='$app_id'") or die(mysqli_error());
		...
		if(mysqli_num_rows($fetch_checkup)>0){ //UPDATE
			$mysqli->query("UPDATE check_ups SET remarks = '$remarks', prescription = '$prescription' WHERE app_id = '$app_id' ") or die(mysql_error());
		}else{ //ADD
			$mysqli->query("INSERT INTO check_ups SET app_id = '$app_id', user_id = '$user_id', remarks = '$remarks', prescription = '$prescription', status='0', date_added = '$date_added'") OR die(mysql_error());
		}
